...
{
	m_backLeft = glm::vec3(-1.0f, 0.0f, -1.0f);
	m_backRight = glm::vec3(1.0f, 0.0f, -1.0f);
	m_Top = glm::vec3(0.0f, 1.0f, 0.0f);
	m_frontLeft = glm::vec3(-1.0f, 0.0f, 1.0f);
	m_frontRight = glm::vec3(1.0f, 0.0f, 1.0f);
	

	//Push back 8 vertices that make up a cube 
	for (int i = 0; i < 3; i++){
		m_vertices.push_back(m_backLeft);
		m_vertices.push_back(m_backRight);
		m_vertices.push_back(m_Top);
		//m_vertices.push_back(m_bottomRightFront);


		m_vertices.push_back(m_frontLeft);
		m_vertices.push_back(m_frontRight);
	...
	// Create Colour list 
	cColours theColour;
	m_colourList[0] = theColour.getColour(theColour.RED);
	m_colourList[1] = theColour.getColour(theColour.RED);
	m_colourList[2] = theColour.getColour(theColour.RED);
	m_colourList[3] = theColour.getColour(theColour.RED);

	m_colourList[4] = theColour.getColour(theColour.ORANGE);
	m_colourList[5] = theColour.getColour(theColour.ORANGE);
	m_colourList[6] = theColour.getColour(theColour.ORANGE);
	m_colourList[7] = theColour.getColour(theColour.ORANGE);

	m_colourList[8] = theColour.getColour(theColour.YELLOW);
	m_colourList[9] = theColour.getColour(theColour.YELLOW);
	m_colourList[12] = theColour.getColour(theColour.YELLOW);
	m_colourList[13] = theColour.getColour(theColour.YELLOW);

	m_colourList[10] = theColour.getColour(theColour.PURPLE);
	m_colourList[11] = theColour.getColour(theColour.PURPLE);
	m_colourList[14] = theColour.getColour(theColour.PURPLE);
	m_colourList[15] = theColour.getColour(theColour.PURPLE);
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * sizeof(m_colourList), &m_colourList[0], GL_STATIC_DRAW);
